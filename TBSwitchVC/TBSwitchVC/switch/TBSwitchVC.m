    ...
    if (object == self.backGroundScrollView) {
        ...
        for (int i = 0; i < self.scrollViewArray.count; i++) {
            if (object == self.scrollViewArray[i]) {
                ...
                CGPoint point = [change[@"new"] CGPointValue];
                [self tb_scrollViewSubContentInset:point drag:scrolllView.isDragging];
                break;
            }
        }
...
- (void)setupChildVC:(UIViewController *)vc {
    ...
    if ([vc respondsToSelector:@selector(tb_expectWhichSrollView)]) {
        [self addExpectWhichSrollView:[vc performSelector:@selector(tb_expectWhichSrollView)]];
    }
...
- (void)tb_scrollViewSubContentInset:(CGPoint)point drag:(BOOL)isDragging {
...
- (void)addExpectWhichSrollView:(UIScrollView *)scrollView {
